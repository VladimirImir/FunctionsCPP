	...
	for (int i = 0; i < ROWS; i++)
	{
...

		{
			for (int k = i; k < ROWS; k++)
			{
				...
				for (int l = k == i ? j + 1 : 0; l < COLS; l++)   //int l = k == i ? j + 1 : 0; l < COLS; l++ 
				{
					if (arr[k][l] < arr[i][j])
					{
						int buffer = arr[i][j];
...
double avg(int arr[ROWS][COLS], const int ROWS, const int COLS)
{
	return (double)sum(arr, ROWS, COLS) / COLS;
}
double avg(double arr[ROWS][COLS], const int ROWS, const int COLS)
{
	return (double)sum(arr, ROWS, COLS) / COLS; //return (double)sum(arr, ROWS, COLS) / (ROWS *COLS);
}
int MinValueIn(int arr[], const int n)
{
	int min = arr[0];
